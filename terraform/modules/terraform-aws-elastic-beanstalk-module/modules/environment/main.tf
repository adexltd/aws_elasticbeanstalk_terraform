...
resource "aws_iam_role_policy_attachment" "service" {
  ...
  role       = join("", aws_iam_role.service[*].name)
  policy_arn = var.prefer_legacy_service_policy ? "arn:aws:iam::aws:policy/aws-service-role/AWSElasticBeanstalkServiceRolePolicy" : "arn:aws:iam::aws:policy/AWSElasticBeanstalkManagedUpdatesCustomerRolePolicy"
}
...
data "aws_iam_policy_document" "default" {
  count = var.enabled ? 1 : 0
  #checkov:skip=CKV_AWS_356:Ensured IAM policy document with `*`
...
resource "aws_elastic_beanstalk_environment" "default" {
  count = var.enabled ? 1 : 0
  #checkov:skip=CKV_AWS_340:Elastic Beanstalk managed platform updates are already enabled
  #checkov:skip=CKV_AWS_312:Elastic Beanstalk environments have enhanced health reporting already enabled
  name                   = var.name
  ...
  setting {
    ...
    name      = "SecurityGroups"
    value     = join(",", sort(compact(concat(var.security_group_ids))))
    resource  = ""
  }
